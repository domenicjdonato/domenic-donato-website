...
We are going to learn about bayesian probabilistic modeling by going through a simple example. Keeping the model simple enables us to perform every step. This will give us a nice understanding of the underlying math in addition to the high level concepts.

If you'd like to follow along via code, you can use this [Colab notebook](https://colab.research.google.com/drive/1REwGPMOk_elQcalsqhKzqQ3WYizB_T37?usp=sharing).
...
We're going to use a problem from [Statistical Rethinking](https://xcelab.net/rm/statistical-rethinking/) by Richard McElreath. In Chapter 4, we are asked to model the distribution of !Kung[^kung] adult heights. This is accomplished by using the library he provides and executing the code
...
``` 
which rewards you with a nice table.
...
From a practical standpoint, this is great, it's easy to use and abstracts away all of the low level details. However, I find that when learning something new, I need to perform every step in order to gain an intuitive understanding of how it works. This post is about exactly that, performing every step in as much detail as we can that gets us from the code snippet to the results in the table.  

## Problem setup

In this section, we describe the data and the model we are going to work with.

### Data

We are provided with census data for the !Kung people, which contains a mix of adults and children. Filtering it to those who were at least 18 years old at the time leaves us with 352 data points. Inspecting the histogram shows that the data is roughly Gaussian.
